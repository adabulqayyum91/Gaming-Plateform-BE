...
//
exports.creatFantasyTeam = async (
	teamViewName,
	userId,
	fantasyLeagueId,
	teamSize,
...
exports.findFantasyTeamDetailByFantasyTeamId = async (fantasyTeamId) => {
	return await FantasyTeam.findOne({
		_id: mongoose.Types.ObjectId(fantasyTeamId),
	});
};
...
exports.findPlayerAllFantayTeam = async (playerIdForPick) => {
	return await FantasyTeam.find({
		"teamMembers.userId": { $in: [mongoose.Types.ObjectId(playerIdForPick)] },
	});
};
exports.dropMemberFromFlTeam = async (flTeamId, flMemberId) => {
	return await FantasyTeam.updateOne(
		...
		{
			$pull: {
				teamMembers: { userId: { $in: [mongoose.Types.ObjectId(flMemberId)] } },
			},
		}
	);
};
	...
	return await FantasyTeam.updateOne(
		{
			_id: mongoose.Types.ObjectId(flTeamId),
			teamMembers: {
				$elemMatch: { userId: mongoose.Types.ObjectId(flMemberId) },
			},
		},
...
exports.updateTradeMovePlayer = async (
	teamIdForUpdate,
	pullPlayerId,
	pushPlayerId
) => {
	await FantasyTeam.updateOne(
		{ _id: mongoose.Types.ObjectId(teamIdForUpdate) },
		{
			$pull: {
				teamMembers: {
					userId: { $in: [mongoose.Types.ObjectId(pullPlayerId)] },
				},
			},
		}
	);
...
exports.findFantasyTeamDeatilByTeamId = async (teamId) => {
	return await FantasyTeam.findOne({
		_id: teamId,
	}).populate(
		"teamOwner",
...
///// for developer
exports.deleteFantasyTeam = async (flTeamId) => {
	return await FantasyTeam.deleteOne({
		_id: mongoose.Types.ObjectId(flTeamId),
	});
};
