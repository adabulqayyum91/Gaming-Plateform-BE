...
exports.tournamentsByNameWithPagination = async (tournamentName, pageNo) => {
	...
	let result = await Tournament.find({
		...
		.limit(pg.pageSize)
		.exec();
	let total = await Tournament.find({
		tournamentName: { $regex: tournamentName },
		isDeleted: false,
	}).countDocuments();
	return {
...
exports.listByTournamentName = async (tournamentName) => {
	return await Tournament.find({
		tournamentName: { $regex: tournamentName.toLowerCase() },
		isDeleted: false,
	});
};
...
exports.updateTournament = async (tournament_, request, id, imagePath) => {
	...
	const updateTournamentInfo = {
		tournamentName:
			request.tournamentName.toLowerCase().trim() || tournament_.tournamentName,
...
exports.addTeam = async (tournamentId, teamId) => {
	return Tournament.updateOne(
		{ _id: mongoose.Types.ObjectId(tournamentId) },
		{ $push: { participatingTeams: teamId } }
	);
};
...
exports.getAllTournamentForGame = async () => {
	return await Tournament.find(
		{ tournamentType: "general", isDeleted: false },
		{ _id: 0, gameToPlay: 1 }
	);
};
exports.getAllTournamentForHome = async () => {
	return await Tournament.find({ tournamentType: "general", isDeleted: false })
		.sort({ createdAt: -1 })
		.limit(4);
};
...
exports.updateWinnerTeam = async (tournamentId, teamId) => {
	return await Tournament.updateOne(
		{ _id: mongoose.Types.ObjectId(tournamentId) },
		{ $set: { winningTeam: mongoose.Types.ObjectId(teamId) } }
	);
};
//for developer

exports.changeTournamentType = async (tournamentId, tournamentType) => {
	await Tournament.updateOne(
		{ _id: mongoose.Types.ObjectId(tournamentId) },
		{ $set: { tournamentType: tournamentType } }
	);
	return true;
};
exports.softDeleteTournament = async () => {
...
exports.updateTournamentDeleteStatus = async (tournamentId, deleteStatus) => {
	await Tournament.updateOne(
		{ _id: mongoose.Types.ObjectId(tournamentId) },
		{ $set: { isDeleted: deleteStatus, deletedAt: null } }
	);
};
exports.showAllTournamentsWithDeleted = async () => {
	return await Tournament.find();
};
