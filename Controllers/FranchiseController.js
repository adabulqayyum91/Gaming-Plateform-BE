...
exports.createFranchise = async (req, res) => {
	...
	if (userId != null) {
		...
		if (!request.franchiseName || !request.occupation || !request.yearlyIncome || !request.address) {
			fs.unlinkSync(imagePath)
			response = ResponseHelper.setResponse(
				ResponseCode.NOT_SUCCESS,
		...
		let franchiseDetailByName = await FranchiseHelper.findFranchiseByNameWithoutDelete(franchiseName)
			...
			for (let k = 0; k < allFranchise.length; k++) {
				for (let m = 0; m < allFranchise[k].franchiseTeams.length; m++) {
					...
					if (teamMemberArr.includes(userId.toString())) {
						fs.unlinkSync(imagePath)
						response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_MEMBER)
						return res.status(response.code).json(response)
					} else {
						let teamMemberStringArr = await TeamContoller.checkFranchiseTeamMemberDetail(userId)//e,n
						if (teamMemberStringArr.includes("e")) {
							fs.unlinkSync(imagePath)
							response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_IN_FRANCHISE_TEAM);
							return res.status(response.code).json(response);
						}
					}
				}
...
exports.getFranchiseByUserTokenOrFranchiseId = async (req, res) => {
	...
	if (userId != null) {
		...
		if (req.query.franchiseId) {
			franchiseData = await this.franchiseDetailDataByFranchiseId(req.query.franchiseId, userId)
		}
		if (!req.query.franchiseId) {
			franchiseData = await this.franchiseDetailData(userId)
		}
		if (Object.keys(franchiseData).length == 0) {
			console.log("*********** Franchise may be either blocked or deleted ************")
			response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.FRANCHISE_BLOCK_OR_DELETED)
			response.franchiseData = franchiseData
			return res.status(response.code).json(response);
		} else {
			response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
...
exports.showAllFranchiseData = async (req, res) => {
	...
	if (userId != null) {
		...
		if (userRole != "Admin") {
			response = ResponseHelper.setResponse(ResponseCode.NOT_AUTHORIZE, Message.NOT_AUTHORIZE_ACTION)
			return res.status(response.code).json(response);
		}
		if (userRole == "Admin") {
			let franchiseObjArr = []
			...
			if (req.query.query) {
				...
				franchiseData = await FranchiseHelper.searchFranchiseByNameWithoutDeleteWithPaginaion(franchiseName, pageNo)
			}
			if (!req.query.query) {
				franchiseData = await FranchiseHelper.showAllFranchiseWithoutDeleteWithPagination(pageNo)
			}
...
exports.approveBlockFranchiseStatus = async (req, res) => {
	...
	if ("approvedStatus" in request) {
		for (let i = 0; i < franchiseIdArr.length; i++) {
			...
			let franchiseBlockStatus = await FranchiseHelper.checkFranchiseBlockStatus(franchiseId)
			if (franchiseBlockStatus == null) {
				response = ResponseHelper.setResponse(
					ResponseCode.NOT_SUCCESS, Message.UNBLOCK_FRANCHISE_FIRST
				)
				return res.status(response.code).json(response);
			} else {
				let franchiseDetailById = await FranchiseHelper.findFranchiseByIdWithoutDelete(franchiseId)
				if (franchiseDetailById == null) {
					notFranchiseId.push(franchiseId)
				}
				// if (franchiseDetailById != null) {
				else {
					await FranchiseHelper.updateApproveStatusByFranchiseId(franchiseId, request.approvedStatus)
	...
	if ("status" in request) {
		...
		await FranchiseHelper.updateBlockStatusByFranchiseId(request.franchiseId, request.status)
	}
	// let pageNo
	// if (request.pageNo) {
	//     pageNo = request.pageNo
	...
	// let franchiseObjData = await this.franchiseObj(franchiseData)
	let franchiseData = await FranchiseHelper.findFranchiseByIdWithoutDeleteWithBlock(franchiseId)
...
exports.userFranchiseTeamsList = async (req, res) => {
	...
	if (userId != null) {
		...
		// }
		let userFanchiseDetail = await this.getUserFranchiseModeAndFranchiseId(userId)
		let franchiseId = userFanchiseDetail.franchiseId
		let franchiseDetail = await FranchiseHelper.findFranchiseByIdWithoutDelete(franchiseId)
...
exports.updateAboutAndStatus = async (req, res) => {
	...
	if (userId != null) {
		if ("about" in request && request.about.length >= 0) {
			let franchiseDetail = await FranchiseHelper.getUserFranchiseDeatil(userId)
			if (franchiseDetail == null) {
				response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.FRANCHISE_NOT_FOUND)
				return res.status(response.code).json(response);
			}
			if (franchiseDetail != null) {
...
exports.allFranchise = async (req, res) => {
	...
	if (userId != null) {
		if (!req.query.franchiseStatus) {
			franchiseData = await FranchiseHelper.showAllFranchiseWithoutDeleteAndBlock()
		}
		if (req.query.franchiseStatus) {
			if (req.query.franchiseStatus.toLowerCase() == "all") {
				franchiseData = await FranchiseHelper.showAllFranchiseWithoutDeleteAndBlock()
			} else {
				franchiseData = await FranchiseHelper.franchiseByStatusWithoutDeleteAndBlock(req.query.franchiseStatus.toLowerCase())
			}
		}
...
exports.franchiseDetailData = async (userId) => {
	...
	if (userFranchiseDetail == null) {
		...
		let formattedCreatedDate = moment(userFranchiseDetail.createdAt).format("MMMM DD, yyyy")
		let franchiseId = userFranchiseDetail._id
		let franchiseTeams = await TeamHelper.findFranchiseTeamWithoutDeleted(franchiseId)
...
exports.franchiseDetailDataByFranchiseId = async (franchiseId, userId) => {
	...
	let franchiseObj
	let tryout
	let tryoutRequestCheck = await TryoutHelper.findSentRequest(franchiseId, userId)
	if (tryoutRequestCheck == null) {
		tryout = ""
	}
	if (tryoutRequestCheck != null) {
...
exports.getUserFranchiseTeam = async (userId) => {
	let userParticipatingFranchiseTeamDetail = await TeamHelper.findUserFranchiseTeam(userId)
	return userParticipatingFranchiseTeamDetail
}
//get user franchise id
exports.getUserUserFrachise = async (teamId) => {
	let userFranchiseDetail = await FranchiseHelper.findUserFranchiseByTeamId(teamId)
...
exports.getUserFranchiseModeAndFranchiseId = async (userId) => {
	let mode
	let franchiseId
	let userFranchiseOwnerCheck = await FranchiseHelper.getUserFranchiseDeatil(userId)
	...
	if (userFranchiseOwnerCheck == null && userFranchiseTeamDetail != null) {
		let userFranchiseDetail = await this.getUserUserFrachise(userFranchiseTeamDetail._id)
