...
exports.listAllUser = async (req, res) => {
	...
	if (userRole == "Admin") {
		...
		if (req.query.query) {
			...
			userList = await UserListHelper.usersListByUserNameWithPagination(
				paramUserName,
				pageNo
			);
		}
...
exports.showAllGames = async (req, res) => {
	...
	if (req.query.query) {
		...
		result = await GameHelper.findGameWithGAmeNameWithPaginationWithoutDel(
...
exports.exportUserData = async (req, res) => {
	...
	if (userId != null) {
		...
		worksheet.getCell("A1").value = "Gaming Plateform";
		...
		worksheet.headerFooter.oddHeader =
			'&C&KCCCCCC&"Aril"&LGaming Plateform&C&A &R&D &T';
