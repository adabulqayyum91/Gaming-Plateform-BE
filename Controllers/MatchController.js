...
exports.showAndSearchMatchForResult = async (req, res) => {
	...
	if (!req.query.query) {
		matchData = await MatchResultHelper.allMatchWithPaginationWithoutDel(
			pageNo
		);
	}
	if (req.query.query) {
		matchData =
			await MatchResultHelper.findMatchByMatchNameWithPaginationWithoutDel(
...
exports.matchAutoExpire = async (req, res) => {
	...
	for (let i = 0; i < pendingMatchList.length; i++) {
		...
		//TODO: for demo 2 mins expiry
		// let hourDurration = moment(
		//     matchCreatedDateTime,
		//     "yyyy-MM-dd hh:mm:ss a"
		// ).fromNow();
		// if (hourDurration.includes("minute") == true) {
		//     if (hourDurration != "a minute ago") {
		//         let min = hourDurration.match(/\d+/)[0];
		//
		//         if (min >= 2) {
		//             await UserHelper.updateCredit(challengeById, challengeByUpdatedCredit)
		//             await MatchHelper.matchInvitationResponse(matchId, "expired")
