			...
			if (dateCheckResult == false) {
				...
				if (alreadyScheduleCheck.length > 0) {
					...
					if (checkLeagueCompleteAndWinnerByRound.length > 0) {
						...
						console.log(checkLeagueCompleteAndWinnerByRound);
						let leagueMatchScheduleDetail = await this.leagueScheduleData(
							request.leagueId
						);
						response = ResponseHelper.setResponse(
							...
							if (previousRoundWinnerTeamsArr.length == 1) {
								...
								if (checkLeagueFinalExist != null) {
									...
									if (leagueDetails.winningTeam == null) {
										...
										let leaguePrize = leagueDetail.prize;
										let winningTeamDetail =
											await TeamHelper.findTeamByIdWithoutDelete(winnigTeamId);
										// console.log("winning team detail : ", winningTeamDetail);
										...
										let prizePerPlayer = leaguePrize / teamMembers;
										// console.log("league prize : ", leaguePrize);
										// console.log("league prize per player : ", prizePerPlayer);
										for (let i = 0; i < winningTeamMemberArr.length; i++) {
											let playerId = winningTeamMemberArr[i].userId;
											let playerDetail = await UserHelper.foundUserById(
												playerId
											);
											let playerOldCredit = playerDetail.userDetail.credits;
											let playNewCredit =
												parseFloat(playerOldCredit) +
												parseFloat(prizePerPlayer);
											await UserHelper.updateCredit(playerId, playNewCredit);
										}
									}
									let winningTeamName =
...
exports.generateLeagueSchedule = async (
	...
	if (scheduleType == "playoff") {
		let maxPlayOffRoundNumberDetail =
			await LeagueScheduleHelper.findMaxPlayOffRoundNumber(leagueId);
	...
	for (let i = 0; i < teamOneArr.length; i++) {
		...
		if (roundNumber > 1) {
			...
			await LeagueScheduleHelper.updateNextMatchId(
				leagueId,
				teamOneArr[i],
				previousRoundNumber,
				id
...
exports.generatePlayOffLeagueSchedule = async (
	...
	console.log("id for second : ", idForSecond);
	srNo = i + 1;
	await LeagueScheduleHelper.updateNextMatchId(
		leagueId,
	...
	await LeagueScheduleHelper.updateWinner(leagueId, firstSlicedTeamArr[0]); //for update winner in qualify team
};
...
exports.leagueScheduleData = async (leagueId) => {
	...
	let leagueMatches = await LeagueScheduleHelper.checkLeagueSchedule(leagueId);
	...
	for (let k = 0; k < maxRoundCount; k++) {
		for (let l = 0; l < leagueMatches.length; l++) {
			if (leagueMatches[l].roundNumber == k + 1) {
				...
				let name;
				if (leagueMatches[l].scheduleType == "playoff") {
					name = "Match_Id : " + matchId;
					// leagueMatches[l].scheduleType +
					// ",round-" +
					// roundNumber +
					// "," +
					// matchId;
				} else if (leagueMatches[l].scheduleType == "final") {
					name = "Match_Id : " + matchId;
					// leagueMatches[l].scheduleType +
					// ",round-" +
					// roundNumber +
					...
					name = "Match_Id : " + matchId;
					// leagueMatches[l].scheduleType + "-" + roundNumber + "," + matchId;
				}
				let resultTextOne;
				let resultTextTwo;
				let isWinnerOne;
...
exports.scheduleForTwoTeams = async (maxRoundNumber, leagueId) => {
	console.log("max round schedule for 2 : ", maxRoundNumber);
	console.log("2 schedule");
	let winnerTeam;
	let firstTeam;
	let secondTeam;
