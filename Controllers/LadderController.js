...
exports.listLadder = async (req, res) => {
	...
	if (!req.query.query) {
		result = await LadderHelper.laddersWithPagination(pageNo);
	}
	let ladderDataArr = [];
		...
		for (let i = 0; i < result.data.length; i++) {
			...
			if (gameDetail != null) {
				...
				let ladderObj = {
					...
					startingDateAndTime: result.data[i].startDateAndTime,
					endingDateAndTime: result.data[i].endDateAndTime,
...
exports.showLaddersForResult = async (req, res) => {
	...
	if (req.query.query) {
		laddersForResult =
			await LadderResultHelper.laddersResultByNameWithPagination(
				ladderName,
				pageNo
			);
	}
...
//// Use with in LadderController - (Sample) ////////////////////////////////////////////////////
